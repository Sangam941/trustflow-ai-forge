...
export default function CreditScorePage() {
  const { score, activities, loading: scoreLoading } = useScore();

  const { data, isLoading } = useQuery({
    queryKey: ["merchantCreditScore"],
    queryFn: () => api.get("/merchant/credit-score")
  });

  useEffect(() => {
    document.title = "Credit Score — Hamisathi";
  }, []);

  if (scoreLoading || isLoading) {
  ...
  const visibleBreakdown = [
    { name: "Financial", value: latest.financialScore || 50 },
    { name: "Bill Payment", value: latest.billPaymentScore || 50 },
    { name: "Transaction", value: latest.transactionScore || 50 },
    { name: "Business Stability", value: latest.businessStabilityScore || 50 },
  ];
      ...
      <div className="rounded-2xl bg-surface p-6 border shadow-soft">
        ...
        <div className="mt-4 space-y-3">
          ...
          <AnimatePresence initial={false}>
            {activities.map((a) => (
              <motion.div key={a._id || a.text} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }}
