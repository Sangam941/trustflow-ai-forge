...
function scoreStatus(score: number) {
  if (score >= 75) return { label: "Excellent", cls: "bg-success/15 text-success border-success/30" };
  if (score >= 65) return { label: "Good", cls: "bg-primary/15 text-primary border-primary/30" };
  if (score >= 55) return { label: "Fair", cls: "bg-warning/15 text-warning border-warning/30" };
...
export default function PayBillsPage() {
  ...
  useEffect(() => {
    document.title = "Pay Bills — Hamisathi";
  }, []);

  const confirm = async () => {
    if (!open) return;
    try {
      await payBill(open._id);
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-2">Based on live financial behavior</p>
          </div>
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <Sparkles className="size-4 text-ai" /> Pay on time: <span className="text-success font-semibold">+2</span> · Late: <span className="text-destructive font-semibold">−3</span>
          </div>
        </div>
      </motion.div>
            ...
            return (
              <motion.div key={b._id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                ...
                <button disabled={isPaid} onClick={() => setOpen(b)}
                  ...
                  {isPaid ? `Paid ${b.paidDate ? "· " + new Date(b.paidDate).toLocaleDateString() : ""}` : "Pay Now"}
                </button>
              </motion.div>
            );
          })}
        ...
        <div className="rounded-2xl bg-surface border shadow-soft p-5">
          ...
          <div className="mt-4 space-y-3">
            ...
            <AnimatePresence initial={false}>
              {activities.map((a) => (
                <motion.div key={a._id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }}
